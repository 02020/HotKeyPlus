...
;------------- 设置轮播的窗体
Toggle_Windows_Set(winId) {
  global GlobalToggleStack
  global GlobalIndex 
  if (!IsObject(GlobalToggleStack))
  {
    GlobalToggleStack := Object()
    GlobalIndex :=1
  }
  WinMinimize, ahk_id %winId%
  GlobalToggleStack.Insert(GlobalToggleStack.Length() +1 ,winId)
}
...
Toggle_Windows() {
  ...
  LastWinId := GlobalToggleStack[GlobalIndex]
  WinMinimize, ahk_id %LastWinId%

  GlobalIndex := GlobalIndex + 1
  if (GlobalToggleStack.Length()  < GlobalIndex)
  {
    GlobalIndex := 1
  } 
  LastWinId := GlobalToggleStack[GlobalIndex]
